...
def action(**changes):
    value = {"protocol_version": "1.0-provisional", "correlation_id": "a-0",
...
def test_expected_role_and_strict_shape(tmp_path):
    session = LiveMatchSession("cop", "game", "session", 1,
                               Journal(tmp_path / "cop.sqlite3"))
    assert session.handle("submit_action_v1", action(sender_role="cop"))["code"] == (
        "WRONG_EXPECTED_ROLE")
    assert session.handle("submit_action_v1", action(extra=True))["code"] == "UNKNOWN_FIELD"


def test_endpoint_policy():
    assert validate_endpoint("http://127.0.0.1:8801/mcp", mode="local",
                             host="127.0.0.1", permitted_port=8801)
    for url in ("https://peer.example/mcp?q=x", "https://127.0.0.1:443/mcp",
                "https://peer.example/wrong"):
        try:
            validate_endpoint(url, mode="remote", host="peer.example",
                              permitted_port=443)
        except ValueError:
            pass
        else:
