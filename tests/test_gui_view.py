...
from salareen_cop.gui_view import load_view
from salareen_cop.report_gui import _draw_heatmap
...
def test_valid_7_by_7_heatmap_renders_49_cells(tmp_path):
    ...
    view = load_view("cop", artifact(tmp_path, heatmap), CONFIG)
    canvas = FakeCanvas()
...
def test_dimension_mismatch_is_rejected(tmp_path):
    with pytest.raises(ValueError, match="row count"):
        load_view("cop", artifact(tmp_path, [[0.0] * 7] * 6), CONFIG)
...
def test_below_minimum_board_is_rejected(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("salareen_cop.gui_view.load_config", lambda path: accepted)
    with pytest.raises(ValueError, match="at least 7x7"):
        load_view("cop", artifact(tmp_path, [[0.0] * 6] * 6), CONFIG)
...
def test_objective_opponent_position_is_not_projected(tmp_path):
    ...
    path = artifact(
        ...
        public_events=[{"event": "turn", "thief_position": [6, 6]}],
    )
    view = load_view("cop", path, CONFIG)
