...
def test_rejected_action_preserves_state_and_scent(tmp_path):
    ...
    session = LiveMatchSession(
        "cop", "game", "session", 1, Journal(tmp_path / "cop.sqlite3"), gameplay
    )
    result = session.handle(
        "submit_action_v1", intent(action_kind="barrier", direction=None, x=3, y=4)
    )
