...
from salareen_cop.live_match.security_runtime import LiveSecurity
from salareen_cop.security.protocol import SecurityViolation

CONFIG = "config/game.json"
PRIVATE_ENV = "SALAREEN_COP_ED25519_PRIVATE_KEY_PATH"
PEER_ENV = "SALAREEN_COP_EXPECTED_PEER_PUBLIC_KEY"
...
def test_restart_preserves_operator_identity(tmp_path, monkeypatch):
    ...
    first = LiveSecurity("cop", CONFIG, "game")
    restarted = LiveSecurity("cop", CONFIG, "game")
...
def test_unexpected_peer_identity_is_rejected(tmp_path, monkeypatch):
    ...
    local = LiveSecurity("cop", CONFIG, "game")
    peer = LiveSecurity("thief", CONFIG, "game")
