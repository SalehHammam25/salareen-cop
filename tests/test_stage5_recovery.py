...
from salareen_cop.cloud_tunneling.models import (
    FailureKind,
    TunnelEndpoint,
    TunnelFailure,
    TunnelReady,
)
from salareen_cop.cloud_tunneling.reconnection import ResumeDecision, ResumeIdentity
from salareen_cop.cloud_tunneling.recovery import recover_connection
from salareen_cop.mcp_transport.orchestrator import PeerOrchestrator
...
def test_matching_recovery_does_not_reapply_acknowledged_message() -> None:
    gateway = PeerOrchestrator("session")
    payload = {
        "protocol_version": "1.0-provisional",
        "correlation_id": "before-reconnect",
        "sender_role": "thief",
        "x": 2,
        "y": 3,
        "step": 4,
    ...
    assert run(Provider([READY])).decision is ResumeDecision.RESUME
    second = gateway.receive(payload)
    assert second == first
