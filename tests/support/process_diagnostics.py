...
def wait_success(peers, timeout: float) -> None:
    ...
    report = [snapshot(peer, timed_out=timed_out[peer.role],
                       cleanup=cleanup[peer.role]) for peer in peers]
    raise AssertionError("peer lifecycle failure:\n" + json.dumps(
...
def wait_event(peer, kind: str, timeout: float = 15,
    ...
    while time.monotonic() < deadline:
        ...
        if peer.exit_code is not None:
            ...
            raise AssertionError(f"{peer.role} exited before {kind}:\n" +
                                 json.dumps(report, indent=2, sort_keys=True))
        time.sleep(0.02)
    report = snapshot(peer, timed_out=True)
    raise AssertionError(f"{peer.role} did not emit {kind}:\n" +
                         json.dumps(report, indent=2, sort_keys=True))
...
def wait_ready(peer, timeout: float = 10) -> None:
    ...
    while time.monotonic() < deadline:
        if peer.exit_code is not None:
            report = snapshot(peer, timed_out=False)
            raise AssertionError("peer exited during readiness:\n" + json.dumps(
                report, indent=2, sort_keys=True))
        ...
        time.sleep(0.02)
    report = snapshot(peer, timed_out=True)
    raise AssertionError("port readiness failed:\n" + json.dumps(
        report, indent=2, sort_keys=True))
