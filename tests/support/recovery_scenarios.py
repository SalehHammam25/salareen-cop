...
def restart(runtime: Path, mode: str, make_peers, canonical) -> dict:
    ...
    try:
        ...
        if mode == "lost_ack":
            ...
            wait_event(
                thief, "stage4_boundary_complete", correlation="action-0", timeout=60
            )
            thief.stop()
            wait_event(cop, "paused")
        thief.extra_env = {}
...
def negative(runtime: Path, mode: str, make_peers, canonical) -> dict:
    ...
    try:
        ...
        kind = {
            "retry_exhaustion": "recovery_exhausted",
            "watchdog": "watchdog_expired",
            "mismatch": "message_rejected",
        }[mode]
...
def terminal_restart(runtime: Path, make_peers, canonical) -> dict:
    ...
    try:
        ...
        start(peers)
        wait_event(
            thief, "stage4_boundary_complete", correlation="action-10", timeout=60
        )
        thief.stop()
        wait_event(cop, "paused")
        thief.start()
