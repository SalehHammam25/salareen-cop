...
1. Start the cop FastMCP process locally on its selected loopback port.
...
4. Exchange the current public URLs out of band with the thief operator. Inject
   the thief URL as `SALAREEN_OPPONENT_URL`; do not commit it.
...
## Second-machine compatibility procedure

On the cop machine, configure `NGROK_DOMAIN=<COP_ASSIGNED_DOMAIN>`, start the
local cop MCP process, and start the tunnel only after local `/mcp` readiness.
On a separate thief machine, perform the symmetric setup with
`<THIEF_ASSIGNED_DOMAIN>`. Configure the cop's private opponent URL as
`https://<THIEF_ASSIGNED_DOMAIN>/mcp` and the thief's private opponent URL as
`https://<COP_ASSIGNED_DOMAIN>/mcp`. Confirm both public `/mcp` endpoints
and bidirectional tool calls before play. Replace placeholders only in private
shells or ignored configuration, never Git.
